...
S9PK_PATH=$(shell find . -name lnbitslegend-lite.s9pk -print)

.DELETE_ON_ERROR:

all: verify

verify: lnbitslegend-lite.s9pk $(S9PK_PATH)
		embassy-sdk verify $(S9PK_PATH)

#install: lnbits.s9pk
#	embassy-cli package install lnbits.s9pk

lnbitslegend-lite.s9pk: manifest.yaml assets/compat/config_spec.yaml config_rules.yaml image.tar docs/instructions.md icon.png $(ASSET_PATHS)
	embassy-sdk pack
